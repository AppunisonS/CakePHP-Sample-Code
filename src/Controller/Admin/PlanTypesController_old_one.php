...
class PlanTypesController extends AppController {
    ...
    public function planTypeListing() {
        ...
        if ($this->request->is('post') && !empty($this->request->data['search'])) {
            $conditions['OR']['PlanTypes.name LIKE'] = '%' . $this->request->data['search'] . '%';
            $conditions['OR']['PlanTypes.price LIKE'] = '%' . $this->request->data['search'] . '%';
            $conditions['OR']['PlanTypes.plan_limit LIKE'] = '%' . $this->request->data['search'] . '%';
        }
        ...
        $Results = $this->paginate($this->PlanTypes->find('All')->contain(['Plans']));
    ...
    public function savePlanType($id = NULL) {
        ...
        if ($this->request->is('post')) {
            if (empty($id)) {
                ...
                $this->request->data['plans'] = json_encode($this->request->data['plans']);
